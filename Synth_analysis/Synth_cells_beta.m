%Synthetic data generating script for cells experiments where correlation is sampled from beta distribution 
clc
clear all
close all
...
cor(1,2,:)=2*betarnd(15,15,N_genes,1)-1;%Data generating correlations
cor(2,1,:)=cor(1,2,:);
H=zeros(size(cor));
for ii=1:size(H,3)
    H(:,:,ii)=Sig*cor(:,:,ii)*Sig;
