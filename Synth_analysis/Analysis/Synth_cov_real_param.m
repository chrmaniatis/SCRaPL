...
TypeOfExp='Coverage/';%Type of experiment
...
lb=[8,15,35,150];
for ii=1:Num_data
File_Meta=sprintf(append(MetaDataFolder,'Synth_real_cov_%d_cnst_Meta.mat'),lb(ii));%Meta filename
File_Data=sprintf(append(RawDataFolder,'Synth_real_cov_%d_cnst.mat'),lb(ii));%Raw data filename
File_Results=sprintf(append(ResultsFolder,'Synth_real_cov_res_%d'),ii);%Save filenaem
