...
DataFolder=[parentdir,'/Downsample_cells/'];%Folder with data
...
ResultsFolder=append('Results/Downsample_cells/',TypeOfExp);%Results folder

Num_data=6;%Number of datasets
lb=[5,10,25,50,100,200];
for ii=1:Num_data
File_Meta=sprintf(append(MetaDataFolder,'Synth_MT_cells_%d_Meta.mat'),lb(ii));%Meta filename
File_Data=sprintf(append(RawDataFolder,'Synth_MT_full_cells_%d.mat'),lb(ii));%Raw data filename
File_Results=sprintf(append(ResultsFolder,'Synth_MT_cells_res_%d'),ii);%Save filenaem
