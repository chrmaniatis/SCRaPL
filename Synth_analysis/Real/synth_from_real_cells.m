...
SaveFolder="Downsample_cells/Data/Cnst";%Save folder
Filename="lt_exp_200.csv";%Latent expression rates filename.
SaveFilename="Synth_real_cells_200.mat";%Synthetic data filename.
...
for ii=1:size(cor_lt_obs,1)
    ...
    scp(ii)=0.1;%Inflation probability
    expre(:,ii)=poissrnd(x2(:,ii));%Generate random poisson data
    expre(tt<scp(ii),ii)=0;%Censor them such that expression distribution follows zero-inflate Poisson.
end
