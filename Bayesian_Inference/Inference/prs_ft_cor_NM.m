...
%in methylation-accessibility data (ie binomail-binomial data)
function [corr_all,p_t] = prs_ft_cor_NM(gns_ind,y_true)

n=histc(y_true(:,5),unique(y_true(:,5)));% number of observations per feature
x1=y_true(:,1)./y_true(:,2);%methylation level 
x2=y_true(:,3)./y_true(:,4);%accessibility level 
