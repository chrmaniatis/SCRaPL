...
%parpool(10)%Start a pool with 32 workers. If parallel computing toolbox please comment that line.
...
File=strcat("NMT_NM_25000_15000_neg1.mat");%Data file name
load(fullfile(DataFolder,File))%loading data

File_meta="NMT_NM_25000_15000_neg1_Meta.mat";%Name of with wchich meta-data will be saved

N_genes=size(unique(y_true(:,5)),1);%Number of genomic regions

%Initializations

%Use data to initialize latent covariance,mean and state.
x_lat=[norminv(0.0000001+0.99999*y_true(:,1)./y_true(:,2)),norminv(0.0000001+0.99999*y_true(:,3)./y_true(:,4))];%Initialize posterior latent states
m_gen=[accumarray(y_true(:,5),x_lat(:,1),[],@mean),accumarray(y_true(:,5),x_lat(:,2),[],@mean)];%Initialize posterior latent means
sig_gen=[accumarray(y_true(:,5),x_lat(:,1),[],@std),accumarray(y_true(:,5),x_lat(:,2),[],@std)];%Initialize posterior latent standard deviations
cor_gen= 2*atanh(prs_ft_cor_NM(y_true(:,5),y_true));%Initialize posterior latent correlations
...
%Prior parameters in clustering component initialization
...
Col_smps=450;%Number of collected samples
Burn_in=1050;%Warm-up steps
...
tic
    ...
    parfor ii=1:N_genes
        ...
        ii 
         ...
         xx(:,1) = drawSamples(smp_x1,'NumSamples',1,'Burnin',10); %sample latent methylation

         log_pdf_x2=@(prmx2) llik_ps_smp_x2_bino(prmx2,yy(:,1:2),m_gen(ii,:),sig_gen(ii,:),cor_gen(ii),xx(:,1))
         smp_x2 = hmcSampler(log_pdf_x2,xx(:,2));
         xx(:,2) = drawSamples(smp_x2,'NumSamples',1,'Burnin',10); %sample latent accessibility
    ...
    m_gen=m_gen1; sig_gen=ssg(:,1:2); cor_gen=ssg(:,3); x_lat=x_col;
    clear m_gen1 ssg x_col
      ...
      if jj>Burn_in
       ...
       [llk1,llk2] = llk_bin(y_true,x_lat,m_gen,sig_gen,cor_gen);
       lg_x_1=llk1;
       lg_x_2=llk2;
      end
      if mod(jj,20)==0    
