...
x_lat=[norminv(0.0000001+0.99999*y_true(:,1)./y_true(:,2)),log(1+y_true(:,3)./norm_fact_all.norm_fact(y_true(:,5))),y_true(:,4)];%Initialize posterior latent states
...
tic
    ...
    parfor ii=1:N_genes
        ...
        if ismember(ii,ign)==1
         ...
         log_pdf_x2=@(prmx2) llik_ps_smp_x2_no_inf(prmx2,yy(:,3),m_gen(ii,:),sig_gen(ii,:),cor_gen(ii),xx(:,1),table2array(norm_fact_all(y_true(:,4)==ii,1))) 
    ...
    clear m_gen1 ssg x_col zero_inf1
      ...
      if jj>Burn_in
       ...
       [llk1,llk2]=llk_ninf(y_true,x_lat,m_gen,sig_gen,cor_gen,zero_inf,norm_fact_all);
       lg_x_1(:,:,ttt)=llk1;
       lg_x_2(:,ttt)=llk2;
      end
      if mod(jj,20)==0    
