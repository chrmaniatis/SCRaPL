---
title: "Gastrulation scNMT-seq: preprocessing and quality control on expression data" #Adapter from
output:
...
setwd("~/SCRaPL/Pre-processing/RNA/")
...
io <- list()
io$gene_metadata <- "Mmusculus_genes_BioMart.87.txt"
io$sample_metadata <- "sample_sheet.csv"#Lab QC
io$counts <- "raw_counts.csv" #Raw expression filename
io$outfile <- "SingleCellExperimentNMT.rds"
io$outfile_flt <- "SingleCellExperimentNMT_filt.rds"
```
...
opts$top50_threshold <- 0.75#0.75      # Maximum fraction of reads accounting for the top 50 features
...
sample_metadata = fread(io$sample_metadata) %>% setkey(sample)#id_rna
```

<!-- Rename sample names in counts -->
```{r}
colnames(counts) <- sample_metadata[colnames(counts),sample]#id_rna
counts <- counts[,sample_metadata$sample]#id_rna
```

```{r load_feature_metadata, echo=FALSE}
feature_metadata <- read.csv(io$gene_metadata, sep="\t", stringsAsFactors=FALSE, quote="", header=T)
...
pdata <- sample_metadata %>% as.data.frame %>% tibble::remove_rownames() %>% tibble::column_to_rownames("sample") %>% .[colnames(counts),]
...
saveRDS(sce_filt,io$outfile_flt)
```
