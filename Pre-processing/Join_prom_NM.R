...
io$out_dir    <- paste0(io$base_dir, "/Join/NM/prom/")
...
acc_data <- acc_data[order(acc_data$feature,acc_data$cell),] %>% setnames(c("acc_gpcs","total_gpcs","feature","cell"))

met_data <- fread(paste0(io$acc_dir,io$acc_data))
...
met_acc <- met_acc[,c(3,4,5,6,1,2)]
met_acc <- met_acc[met_acc$met_cpgs>-1 & met_acc$acc_gpcs>-1,]
met_acc <- met_acc[order(met_acc$feature,met_acc$cell),]
