...
io$out_dir    <- paste0(io$base_dir, "/met/processed/")#change met to acc for accessibility data
io$annos_file <- paste0(io$base_dir, "/annotations/", io$anno_name, ".bed")
io$met_dir    <- paste0(io$base_dir, "/met/binarised/")#change met to acc for accessibility data    
...
for (m_file in io$met_files) {
    ...
    cell <-gsub("/home/christos/Desktop//MTDATA/met//binarised/","\\1",m_file)
